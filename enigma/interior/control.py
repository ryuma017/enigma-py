...
class PlugBoard():
    ...
    def __init__(self, pairs=None):
        self.wiring = self.__wire_pairs(pairs)
        self.forward_map = {}
        self.backward_map = {}
        self.__set_wiring_map(self.wiring)
        super().__init__()

    def __wire_pairs(self, pairs):
        if pairs is None:
            return LETTERS
    ...
    def __set_wiring_map(self, wiring):
    ...
    def convert_forward(self, index):
        ltr = LETTERS[index]
        ltr = self.forward_map.get(ltr)
        return LETTERS.index(ltr)

    def convert_backward(self, index):
        ltr = LETTERS[index]
        ltr = self.backward_map.get(ltr)
        return LETTERS.index(ltr)
